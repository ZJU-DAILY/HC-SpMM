...
__device__ __forceinline__ void inplace_deduplication_gpu(int *array, int length,
                                                      int *loc) {
  int cur = 1;
  while (cur < length) {
...
std::vector<torch::Tensor> preprocess(torch::Tensor edgeList_tensor, 
    ...
    auto d_edgeList = edgeList_tensor.data<int>();
    auto d_nodePointer = nodePointer_tensor.data<int>();
    ...
    generate_edgetocolumn_cuda(
        d_nodePointer, d_edgeList, thrust::raw_pointer_cast(&deviceEL[0]), d_edgeToColumn,
        d_blockPartition, block_num, BLK_H, BLK_W, num_nodes, d_hybrid_type);
